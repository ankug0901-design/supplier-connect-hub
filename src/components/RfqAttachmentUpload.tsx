...
const ACCEPTED_EXT = ['pdf', 'ppt', 'pptx', 'jpg', 'jpeg', 'png', 'ai', 'psd', 'doc', 'docx'];
const ACCEPT_ATTR = '.pdf,.ppt,.pptx,.jpg,.jpeg,.png,.ai,.psd,.doc,.docx';
const MAX_BYTES = 25 * 1024 * 1024;
// 10 years in seconds. Signed URL is used because the workspace blocks public buckets.
const SIGNED_URL_EXPIRY = 60 * 60 * 24 * 365 * 10;

interface Props {
  folder: string; // e.g. rfq_id or "new_<timestamp>"
...
export function RfqAttachmentUpload({ folder, prefix, onUploaded, disabled }: Props) {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const path = `${folder}/${prefix ? prefix + '_' : ''}${ts}_${safeName}`;
...
export function UploadedFileBadge({ name, onClear }: { name: string; onClear?: () => void }) {
  return (
