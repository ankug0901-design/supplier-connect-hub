...
function AttentionBanner({ a }: { a: ReturnType<typeof useDashboardData>['data']['attention'] }) {
  ...
  const chips: { count: number; bg: string; fg: string; label: string; icon: React.ReactNode; to: string }[] = [
    ...
    { count: a.three_way_match_exceptions, bg: '#E0E7FF', fg: '#3730A3', label: `${a.three_way_match_exceptions} three-way match exceptions`, icon: <AlertTriangle className="h-3 w-3" />, to: '/admin/three-way-match?tab=mismatch' },
  ].filter((c) => c.count > 0);
